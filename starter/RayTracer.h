...
struct object3D {
  ...
  // Below we set up space for a pointer to the intersection function for this
  // object. Note that the intersection function must compute the lambda at the
  // intersection, the intersection point p, the normal at that point n, and the
  // texture coordinates (a,b). The texture coordinates are not used unless
  // texImg!=NULL and a textureMap function has been provided
  void (*intersect)(struct object3D *obj, struct ray3D *ray, double *lambda,
                    struct point3D *p, struct point3D *n, double *a, double *b);

  // Texture mapping function. Takes normalized texture coordinates (a,b) and
  // returns the texture colour at that point using bi-linear interpolation
  ...
  // uniformly random-sampled point on the surface. These are needed for Photon
  // Mapping.
  void (*surfaceCoords)(struct object3D *obj, double a, double b, double *x,
                        double *y, double *z);
  ...
  int isCSG;  // Object is part of a CSG composite object. Links to components
              // via CSGnext
...
// Raytracing
void buildScene(
    void);  // Scene set up. Defines objects and object transformations
