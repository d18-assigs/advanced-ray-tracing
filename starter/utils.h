...
inline void matScaMult(double A[4][4],double scalar)
{
 // Matrix scalar multiplication A=s*A, notice that the result
 // is left in A. This is useful for performing transformations
 // on points and rays
 double C[4][4];
 int i,j,k;
...
inline void matAdd(double A[4][4],double B[4][4])
{
 // Matrix Matrix addition B=A+B, notice that the result
 // is left in B. This is useful for performing transformations
 // on points and rays
 double C[4][4];
 int i,j,k;
...
inline void scaleVectors(struct point3D *a, double scaler){
  a->px=a->px * scaler;
  a->py=a->py * scaler;
  a->pz=a->pz * scaler;
  a->pw=1;
...
inline void assignPoint(struct point3D *p0, double x, double y, double z){
  // assign a point with value x y z w = 1;
  p0->px = x;
  p0->py = y;
  p0->pz = z;
  p0->pw = 1;
...
inline void initRay(struct ray3D *ray, struct point3D *p0, struct point3D *d, double last_r_index)
{
...
void cylCoordinates(struct object3D *cylinder, double a, double b, double *x, double *y, double *z);
void coneCoordinates(struct object3D *cone, double a, double b, double *x, double *y, double *z);
...
void cylSample(struct object3D *cylinder, double *x, double *y, double *z);
